...
//
// FIX NOTE: Card is defined OUTSIDE SignupPage so React doesn't treat it
// as a new component on every render. If it was inside, every keystroke
// would cause React to destroy + rebuild the Card, killing input focus.
...
// ── CARD WRAPPER ──
// Defined outside SignupPage so its identity is stable across renders.
// If this was inside SignupPage, every state change would create a new
// Card function → React unmounts old Card → inputs lose focus.
function Card({
  step,
  error,
  children,
            ...
            return (
              <div key={s} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <div style={{
                  ...
                  border: `1px solid ${done ? '#22c55e' : active ? 'var(--accent)' : 'var(--border)'}`,
                  transition: 'all 0.2s',
                }}>
                  {done ? '✓' : i + 1}
                </div>
...
export default function SignupPage() {
  ...
  async function handleSetPassword(e: React.FormEvent) {
    ...
    // Also update profiles table directly in case the trigger already ran
  ...
  // Password strength: 1 = weak, 2 = ok, 3 = good, 4 = strong
  ...
  return (
    <Card step={step} error={error}>
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ fontSize: '13.5px', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
          One last step — set a password for your Forge account.
        </p>
      </div>
            ...
            <div style={{ display: 'flex', gap: '4px', marginTop: '8px' }}>
              {[1, 2, 3, 4].map(level => (
                <div key={level} style={{
                  ...
                  background: level <= passwordStrength ? strengthColor : 'var(--border)',
                  transition: 'background 0.2s',
                }} />
              ))}
            </div>
